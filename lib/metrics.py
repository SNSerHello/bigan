...
def cv_reg_lr(
    ...
    vaX,
    vaY,
    Cs=[0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0],
    return_score=True,
):
    tr_accs = []
    va_accs = []
    ...
    if return_score:
        return 100 * va_accs[best]
    print(
        "best model C: %.4f tr_acc: %.4f va_acc: %.4f"
...
def gpu_nnc_predict(trX, trY, teX, metric="cosine", batch_size=4096):
    ...
    for i in range(0, len(teX), batch_size):
        ...
        for j in range(0, len(trX), batch_size):
            dist = metric_fn(
                floatX(teX[i : i + batch_size]), floatX(trX[j : j + batch_size])
            )
            if metric == "cosine":
