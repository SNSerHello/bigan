...
class Regularizer(object):
    ...
    def ignored(self, p):
        return (p.name is not None) and (
            any(p.name.startswith(pre) for pre in self.ignored_prefixes)
...
class Update(object):
    def __init__(self, regularizer=Regularizer(), clipnorm=0.0):
        self.__dict__.update(locals())
...
class Adadelta(Update):
    ...
    def __call__(self, params, cost):
        ...
        for p, g in zip(params, grads):
            ...
            acc = theano.shared(p.get_value() * 0.0)
            acc_delta = theano.shared(p.get_value() * 0.0)
            acc_new = self.rho * acc + (1 - self.rho) * g**2
            updates.append((acc, acc_new))

            update = (
                g * T.sqrt(acc_delta + self.epsilon) / T.sqrt(acc_new + self.epsilon)
            )
