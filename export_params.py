...
def transplant_weights(weights, caffenet, flip_filters=True, reverse_3ch=True):
    ...
    mismatched = None
    num_layers = 0
    for (name, caffe_weights), theano_weights in zip(caffenet.params.items(), weights):
        ...
        if len(theano_weights) > 1 and len(caffe_weights) == 1:
            print(
                (
                    'Layer "%s" did not match: '
                    "Theano had bias; Caffe layer had only weights"
                )
                % name
            )
            mismatched = name
...
if __name__ == "__main__":
    ...
    parser.add_argument("weights", help="(*.jl) weights file saved by train_gan.py")
