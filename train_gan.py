...
parser.add_argument(
    "--discrim_weight", type=float, default=1, help="Weighting of discrim loss"
)
parser.add_argument(
    "--joint_discrim_weight",
    type=float,
    default=0,
...
parser.add_argument(
    "--no-encode", action="store_false", dest="encode", help="No encoder"
)
parser.add_argument(
    "--encode_weight",
    type=float,
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Don't update both nets together, even if k==1 "
...
parser.add_argument(
    "--bn_separate", action="store_true", help="Apply batch norm to x, z separately"
)
...
parser.add_argument("--gen_net", help="Name of network architecture for generator")
parser.add_argument(
    "--gen_net_size",
    type=int,
...
parser.add_argument(
    "--deconv_nonlin", default="ReLU", help="Nonlinearity for deconv nets"
)
parser.add_argument(
    "--deconv_ksize", type=int, default=5, help="Kernel size for deconv nets"
)
...
parser.add_argument("--conv_nonlin", default="LReLU", help="Nonlinearity for conv nets")
parser.add_argument(
    "--net_fc_drop",
    type=float,
    default=0,
...
parser.add_argument(
    "--cond_fc_drop", type=float, help="The dropout probability for the cond FC layers"
)

# Discriminator
parser.add_argument(
    "--discrim_net", help="Name of network architecture for discriminator"
)
parser.add_argument(
    "--discrim_net_size",
    type=int,
...
parser.add_argument(
    "--cat_inputs",
    action="store_true",
    help="Concatenate real/gen images input to discrim+encoder " "(affects batch norm)",
)

# Encoder
parser.add_argument("--encode_net", help="Name of network architecture for encoder")
parser.add_argument(
    "--encode_nonlin",
...
# Data
parser.add_argument("--dataset", default="mnist", help="Dataset (mnist/imagenet)")
parser.add_argument(
    "--raw_size", type=int, help="Raw minor edge size (for image datasets)"
)
parser.add_argument(
    "--crop_size", type=int, default=28, help="Crop size (for image datasets)"
)
parser.add_argument(
    "--crop_resize",
    type=int,
...
parser.add_argument(
    "--max_labels", type=int, help="If set, include only labels 0:(max_labels - 1)"
)
parser.add_argument(
    "--max_images", type=int, help="If set, include at most this many images"
)
parser.add_argument(
    "--use_test_set",
    action="store_true",
...
parser.add_argument(
    "--optimizer", default="adam", help="Optimization algorithm (adam, sgd, or rms)"
)
parser.add_argument(
    "--discrim_optimizer",
...
parser.add_argument(
    "--batch_size", type=int, default=128, help="Batch size used for learning"
)
parser.add_argument(
...
parser.add_argument("--decay", type=float, default=2.5e-5, help="L2 weight decay")
parser.add_argument(
    "--discrim_decay",
    type=float,
...
parser.add_argument(
    "--momentum", type=float, default=0.5, help="Momentum coeff for Adam/SGD"
)
parser.add_argument(
    "--epochs",
    type=int,
    default=50,
    help="Number of epochs to train before beginning LR decay " "(doubled if k != 1)",
)
parser.add_argument(
    "--decay_epochs",
    type=int,
...
parser.add_argument(
    "--exp_dir", default="exp", help="Directory where experiment results are stored"
)
parser.add_argument(
    "--disp_interval",
    type=int,
...
parser.add_argument(
    "--disp_samples", type=int, default=200, help="Max number of samples to display"
)
parser.add_argument(
    "--megabatch_gb",
    type=float,
...
parser.add_argument(
    "--classifier_deploy", action="store_true", help="Learn classifier at deploy time"
)

# Model saving / loading
parser.add_argument(
    "--save_interval", type=int, help="Save params every N epochs for this N"
)
parser.add_argument(
    "--resume", type=int, help="Epoch from which to resume training and load weights"
)
parser.add_argument(
    "--weights", help="Weights filename prefix (e.g., * in *_discrim_params.jl)"
)
...
if args.encode_net_fc_dims is not None:
    args.encode_net_fc_dims = [int(d) for d in args.encode_net_fc_dims.split(",") if d]
if args.cond_fc_dims is not None:
    args.cond_fc_dims = [int(d) for d in args.cond_fc_dims.split(",") if d]
...
Xs = Output(
    input_transform(Xis), (args.batch_size, nc, args.crop_resize, args.crop_resize)
)
Z = dist.placeholders
if args.classifier:
    Y = T.ivector()
...
# generation
gen_kwargs = dict(
    args=args, dist=dist, nc=nc, bnkwargs=bnkwargs, gen_transform=gen_transform
)
...
ignored_prefixes = []
if args.no_decay_bias:
    ignored_prefixes.append("Bias")
if args.no_decay_gain:
    ignored_prefixes.append("Gain")
...
discrim_reg = updates.Regularizer(
    l2=args.discrim_decay, ignored_prefixes=ignored_prefixes
)

if args.discrim_optimizer is None:
    args.discrim_optimizer = args.optimizer
discrim_updater = get_updater(args.discrim_optimizer, lr=lrt, regularizer=discrim_reg)


def featurizer(x=None, gx=None, args=args, **kwargs):
    return gan.Featurizer(
        args, dist, x, gx, y, nc=nc, ny=ny, bnkwargs=bnkwargs, updater=updater, **kwargs
    )
...
if args.discrim_weight:
    f_discrim = discrim_featurizer(
        ...
        net_name=args.discrim_net,
        net_size=args.discrim_net_size,
        is_discrim=True,
        name="Discriminator",
    )
...
if args.joint_discrim_weight:
    ...
    def joint_discrim(X, Xs, Y, Z_dist, name="JointDiscriminator"):
        ...
        discrim = discrim_featurizer(
            x=Xs,
            gx=gX,
            args=joint_discrim_args,
    ...
    f_encoder.net.add_loss(
        f_joint_discrim.net.get_loss("opp_loss_real"), weight=weight, name="loss_real"
    )
    f_encoder.net.add_loss(
        f_joint_discrim.net.get_loss("opp_loss_gen"), weight=weight, name="loss_gen"
    )
...
if args.encode:
    ...
    if args.encode_gen_weight:
        try:
            train_gen.net.add_loss(net.get_loss(), weight=args.encode_gen_weight)
        except KeyError:
...
g_updates = train_gen.net.get_updates(updater=updater, extra_params=encode_gen_params)
...
if args.classifier:
    if f_discrim is not None:
        ...
        _discrim_preds = lazy_function([discrim_feats.value], discrim_preds.value)
...
if update_both:
    ...
    _train_gd = lazy_function(inputs, [], updates=all_updates, on_unused_input="ignore")
else:
    niter *= 2
    niter_decay *= 2
    ...
    _train_g = lazy_function(
        inputs, [], updates=train_g_updates, on_unused_input="ignore"
    )
...
if args.encode:
    ...
    _enc_sample = lazy_function(
        X_enc_input, [o.value for o in dist.logits_to_sample(enc_preds)]
    )
    ...
    if args.classifier:
        ...
        _enc_preds = lazy_function([enc_feats.value], enc_label_preds.value)
...
megabatch_size = min(
    max_images_per_megabatch, int(max_memory_per_megabatch / float(image_bytes))
)
...
gY = np.asarray(np_rng.randint(0, ny, mega_num_samples), dtype=np.int32).flatten()
...
sample_y = np.array(
    [[i] * num_sample_cols for i in range(ny)], dtype=np.int32
).flatten()
...
def eval_and_disp(epoch, costs, ng=(10 * megabatch_size)):
    ...
    cost_string = "  ".join("%s: %.4f" % o for o in zip(disp_costs.keys(), costs))
    ...
    if args.encode:
        ...
        def image_recon_error(enc_inputs, recon_sized_inputs=None):
            ...
            errors = batch_map(
                _f_error, [enc_inputs, recon_sized_inputs], wraparound=True
            )
    ...
    if args.encode:
        if args.crop_size == args.crop_resize:
            ...
            dataset.grid_vis(
                inverse_transform(samples_regen), sample_shape, imname("regen")
            )
...
def load_params(weight_prefix=None, resume_epoch=None, groups=param_groups):
    ...
    for key, param_list in groups.iteritems():
        ...
        if len(saved_params) != len(param_list):
            raise ValueError(
                ("different param list lengths: " "len(saved)=%d != %d=len(params)")
        ...
        for saved, shared in zip(saved_params, param_list):
            if shared.get_value().shape != saved.shape:
                raise ValueError(
                    ("shape mismatch: " "saved.shape=%s != %s=shared.shape")
...
def set_lr(
    epoch, lr=args.learning_rate, final_lr=args.final_lr_mult * args.learning_rate
):
    if epoch < niter:  # constant LR
        ...
        if args.linear_decay:
            ...
            log_value = np.log(lr) + proportion_complete * (
                np.log(final_lr) - np.log(lr)
            )
...
def train():
    ...
    save_epochs = frozenset([i for i in [niter, total_niter] if i > start_epoch])
    ...
    for epoch in xrange(start_epoch, total_niter + 1):
        ...
        print("Epoch %d: %f seconds (LR = %g)" % (epoch, epoch_time, lrt.get_value()))
