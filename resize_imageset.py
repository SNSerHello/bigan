...
def resize_image(
    ...
    if verbose:
        print(
            "Resized image (%s) from %s to %s; saving to file: %s"
...
def resize_directory(
    minor_size, in_root, out_root, pool=FakePool(), verbose=False, **kwargs
):
    if verbose:
        print("Finding files in input dir: ", in_root)
    ...
    if verbose:
        ...
        print(
            "\n".join(
                "\t" + f for f in (files[:k] + (["..."] if len(files) > k else []))
            )
        )
    ...
    if verbose:
        print("Done.")
    resize = functools.partial(resize_image, minor_size, in_root, out_root, **kwargs)
    if verbose:
        print("Resizing %d images." % len(files))
    iterator = pool.imap_unordered(resize, files, chunksize=10)
    for _ in tqdm(iterator, total=len(files)):
        pass
    if verbose:
        print("Done.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--format",
        help='Output format, e.g. "png" or "jpg" ' "(if unspecified, use input format)",
    )
    ...
    parser.add_argument("size", type=int, help="The minor edge size of the output")
    parser.add_argument("input_directory", help="Directory containing input images")
    parser.add_argument(
        "output_directory", help="Directory to be filled with output images"
    )
    args = parser.parse_args()
    if not args.quiet:
        print("Opening pool with %d workers" % args.jobs)
    if args.jobs == 0:
        pool = FakePool()
    elif args.jobs >= 1:
