...
class UniformDistribution(Distribution):
    ...
    def __init__(
        self, num, definition, limits=default_limits, internal_rng=False, name=None
    ):
        assert len(limits) == 2
        ...
        if len(definition) != 1:
            raise ValueError(
                "definition should have 1 parameter (dim), not %d" % len(definition)
            )
        try:
    ...
    def sample(self, num=None):
        if num is None:
            num = self.num
        return [floatX(np.random.uniform(size=(num, self.sample_dim), **self.rangekw))]
...
class GaussianDistribution(Distribution):
    ...
    def sample(self, num=None):
        ...
        return [
            floatX(
                np.random.normal(
                    loc=self.mean, scale=self.stdev, size=(num, self.sample_dim)
                )
            )
        ]
...
class MultiDistribution(Distribution):
    known_dists = frozenset(
        d
        for d in globals().values()
        if type(d) == type and issubclass(d, Distribution) and hasattr(d, "nickname")
    )
    ...
    def __init__(
        ...
        assert len(weights) == len(self.dists), "weights length must be the # of dists"
        assert not any(w < 0 for w in weights), "weights must have no negative entries"
        ...
        if normalize:
            ...
            self.weights = [
                float(w) / sum_weights / d.norm_divisor()
    ...
    def _parse_def(self, definition):
        ...
        for dist_def in definition.split("_"):
            ...
            try:
                ...
                known_nicknames = "\n".join(
                    "\t{}\t{}".format(nick, t.__name__) for nick, t in mapping
                )
        ...
        for d, r in zip(self.dists, self.recon_slice(recon)):
            try:
                ...
                if skip_unimplemented:
                    print(
                        'Warning: op "%s" not implemented for '
    ...
    def dist_sized_slices(self, input_list):
        ...
        return chain([[i] * len(d.flat_data) for i, d in zip(input_list, self.dists)])
