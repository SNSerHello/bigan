...
class Layer(object):
    def __init__(self, *inputs, **kwargs):
        ...
        for index, out in enumerate(outs):
            ...
            if out.shape is None:
                ...
                input_dict = {
                    i.value: np.zeros(i.shape, dtype=i.value.dtype)
                    for i in inputs
                    if not isinstance(i.value, skip_types)
                }
                out_shape = out.value.shape.eval(input_dict)
                outs[index] = Output(out.value, out_shape, index_max=out.index_max)
        self.output = tuple(outs)
        print(
            "(%s) Creating outputs with shapes: %s"
    ...
    def weights(
        ...
        if reparameterize:
            ...
            g = self.add_param(
                f_init(g_shape, dtype=theano.config.floatX), prefix="w_scale"
            )
            w = reparameterized_weights(w, g, exp=exp_reparam, nin_axis=nin_axis)
        return w
    ...
    def bn_count(self):
        return self.add_param(
            np.zeros(()), prefix="count", learnable=False, dtype="int"
        )
...
class Concat(Layer):
    def get_output(self, *h, **kwargs):
        ...
        if len(num_axes) != 1:
            raise ValueError("Concat: inputs have differing ndims: %s" % (num_axes,))
        num_axes = num_axes.pop()
        if not (0 <= axis < num_axes):
            raise ValueError(
                ("Concat: must have 0 <= axis (= %d) " "< num_axes (= %d)")
                % (num_axes, axis)
            )
        ...
        for i in xrange(num_axes):
            ...
            if len(dims) != 1:
                raise ValueError(
                    "Concat: differing axis %d dimensions: %s" % (i, sorted(dims))
                )
...
class Slice(Layer):
    def get_output(self, h, axis=1, num=None, slice_point=None):
        ...
        slices = [
            slice(start, end)
            for start, end in zip([0] + slice_point, slice_point + [in_dim])
        ]
        pad = [slice(None)] * axis
        return tuple(Output(h.value[pad + [s]], index_max=h.index_max) for s in slices)
...
class Conv(Layer):
    def get_output(
        ...
        assert nin % group == 0
        W = self.weights(
            (nout, nin // group, ksize, ksize), stddev=stddev, nin_axis=[1, 2, 3]
        )
        ...
        for g in xrange(group):
            ...
            outs.append(
                T.nnet.conv2d(
                    hi, w, border_mode=pad, subsample=subsample, filter_flip=filter_flip
                )
            )
        if len(outs) == 1:
            out = outs[0]
...
def deconv(h, w, subsample=(1, 1), border_mode=(0, 0), out_dims=None, filter_flip=True):
    if out_dims is None:
...
class Deconv(Layer):
    def get_output(self, h, nout=None, ksize=1, stride=1, pad="SAME", stddev=None):
        if nout is None:
            raise ValueError("nout must be provided")
        h, h_shape = h.value, h.shape
        ...
        if out_shape_specified:
            ...
            out_size = (
                stride * (s - 1) + ksize + (ksize % 2) - 2 * pad for s in h_shape[2:]
            )
        ...
        if len(h_shape) == 2:
            h_shape += 1, 1
            h = h.reshape(*h_shape)
        expected_input_shape = conv_out_shape(
            in_shape=out_shape, nout=nin, ksize=ksize, stride=stride, pad=pad
        )
        if h_shape != expected_input_shape:
            raise ValueError(
                (
                    "deconv: input shape %s does not match expected "
                    "input shape %s for output shape %s"
                )
                % (h_shape, expected_input_shape, out_shape)
            )
        W = self.weights((nin, nout, ksize, ksize), stddev=stddev, nin_axis=[0, 2, 3])
...
class Pool(Layer):
    def get_output(self, h, ksize=1, stride=1, pad="SAME", mode="max"):
        ...
        assert len(h_shape) == 4
        pad = get_pad(pad, ksize)
        out = dnn_pool(h, ws=(ksize, ksize), stride=(stride, stride), pad=(pad, pad))
        return Output(out)
...
class FC(Layer):
    def get_output(
        self, h, nout=None, stddev=None, reparameterize=reparam, exp_reparam=exp_reparam
    ):
...
class OneHot(FC):
    ...
    def get_output(self, h, **kwargs):
        ...
        if not isinstance(nin, int):
            raise TypeError(
                "h.index_max must be an integer; was: %s (type %s)" % (nin, type(nin))
            )
        shape = nin, nin
...
class BatchNorm(Layer):
    def get_output(
        ...
        ave_frac=1,
        epsilon=1e-8,
        log_var_move_ave=False,
        var_bias_correction=True,
        ignore_moment_grads=False,
    ):
    ...
    def get_output(self, h, ratio=0.5):
        ...
        def op(h):
            if ratio == 0:
                return h
            keep_prob = 1 - ratio
            mask = t_rng.binomial(h.shape, p=keep_prob, dtype=theano.config.floatX)
...
class Net(object):
    ...
    def get_loss(self, name="loss"):
        if self.is_agg_loss[name]:
            return sum(
                w * self.get_loss(k).mean() for k, w in self.agg_loss_terms[name]
            )
        no_grad = theano.gradient.disconnected_grad
    ...
    def _add_layer(self, layer_constructor, *args, **kwargs):
        ...
        name = "%s%s%d" % (self.name_prefix, type_name, self.layer_count[type_name])
    ...
    def _add_param(
        self, name, value, learnable=True, layer_name="", dtype=theano.config.floatX
    ):
        if self.reuse:
            ...
            if value.shape != existing_shape:
                raise ValueError(
                    'Param "%s": incompatible shapes %s vs. %s'
                    % (name, existing_shape, value.shape)
                )
            print(
                '(%s) Reusing param "%s" with shape: %s'
                % (layer_name, name, value.shape)
            )
        else:
            print(
                '(%s) Adding param "%s" with shape: %s'
                % (layer_name, name, value.shape)
            )
    ...
    def __getattr__(self, attr):
        def layer_method(*args, **kwargs):
            return self._add_layer(self.layer_types[attr], *args, **kwargs).output
...
def batch_norm(
    N, h, batch_norm=True, bias=False, gain=False, log_gain=False, use_ave=False
):
    if batch_norm:
        h = N.BatchNorm(h, use_ave=use_ave)
    if gain:
        h = N.Gain(h, log_gain=log_gain)
    if bias:
        h = N.Bias(h)
    return h
...
def multifc(N, H, nout=None, renormalize_weights=True, **kwargs):
    ...
    weighted_outputs = [
        N.Scale(o, scale=w) for o, w in zip(unweighted_outputs, weights)
    ]
...
def deconvnet_128(
    ...
    # h \in (size*8, 4, 4)
    h = deconv_acts(
        h, nout=(size * 4, ss * 2, ss * 2), stride=2
    )  # g_h1 (size*4,   8,   8)
    h = deconv_acts(
        h, nout=(size * 2, ss * 4, ss * 4), stride=2
    )  # g_h2 (size*2,  16,  16)
    h = deconv_acts(
        h, nout=(size * 1, ss * 8, ss * 8), stride=2
    )  # g_h3 (size*1,  32,  32)
    h = deconv_acts(
        h, nout=(size // 2, ss * 16, ss * 16), stride=2
    )  # g_h4 (size*1,  64,  64)
...
def convnet(
    ...
    cond_fc_dims=[],
    cond_fc_drop=0,
    minibatch_layer_halves=False,
    minibatch_layer_size=None,
    ...
    def acts(h, ksize=1, do_cond=True):
        if do_cond:
            h = apply_cond(N, h, cond=cond, ksize=ksize, bn=bn, bn_separate=bn_separate)
        h = nonlin(h)
        return h
    ...
    if minibatch_layer_size is not None:
        ...
        h = Output(
            T.concatenate([hin.value, diff_mean], axis=1),
