    ...
    def get_data(self, num):
        ...
        for _ in range(num_batches):
            ...
            if out_data is None:
                bd, bl = out_batch
                out_data = [
                    np.zeros((total_num,) + b.shape[1:], dtype=b.dtype) for b in bd
                ]
...
def pil_random_crop(image, crop):
    ...
    return image.crop((offset[1], offset[0], offset[1] + crop[1], offset[0] + crop[0]))
...
def get_image(
    ...
    if minor_size is not None:
        ...
        new_size = tuple(int(round(scale_factor * s)) for s in image.size)
    ...
    downsampled_images = [
        np.array(image.resize(s, Image.ANTIALIAS)) for s in crop_shapes[1:]
    ]
...
class ImageDataProvider(DataProvider):
    ...
    def __init__(
        ...
        if labels is not None:
            ...
            data = [(image, label_to_index[l]) for image, l in data if l in label_set]
        if (max_images is not None) and (len(data) > max_images):
            print(
                "Shrinking dataset from %d images to %d images"
                % (len(data), max_images)
            )
        ...
        self.out_data = [
            np.zeros((batch_size,) + s, dtype=np.uint8) for s in self.image_shapes
        ]
...
tinyvidbeach_data_providers = functools.partial(
    imagenet_data_providers, root="data/tinyvidbeach"
)
tinyvidgolf_data_providers = functools.partial(
    imagenet_data_providers, root="data/tinyvidgolf"
)
cityscapes_data_providers = functools.partial(
    imagenet_data_providers, root="data/cityscapes", num_test=None
)


class MemoryDataProvider(DataProvider):
    def __init__(self, data, label, batch_size, image_shape=None, crop_size=None):
        for x in [data, label]:
            assert isinstance(x, np.ndarray)
        assert len(crop_size) == 1
        crop_size = crop_size[0]
        (self.data, self.labels, self.batch_size, self.crop_size, self.image_shape) = (
            data,
            label,
            batch_size,
            crop_size,
...
def mnist_data_providers(batch_size, crop_size=[], use_test_set=False):
    ...
    shape = 1, 28, 28
    return {
        "train": MemoryDataProvider(
            trX, trY, batch_size, crop_size=crop_size, image_shape=shape
        ),
        "val": MemoryDataProvider(
            vaX, vaY, batch_size, crop_size=crop_size, image_shape=shape
        ),
        "test": MemoryDataProvider(
            teX, teY, batch_size, crop_size=crop_size, image_shape=shape
        ),
    }


def pong_data_providers(
    batch_size, crop_size=[], filename="./data/atari/Pong-100000-dqn-dec.h5"
):
    import h5py
    ...
    shape = 4, 84, 84
    return {
        s: MemoryDataProvider(
            data[i], labels[i], batch_size, crop_size=crop_size, image_shape=shape
        )
        for s, i in [("train", train_idx), ("val", val_idx), ("test", val_idx)]
    }


spaceinv_data_providers = functools.partial(
    pong_data_providers, filename="./data/atari/SpaceInvaders-100000-dqn-dec.h5"
)
seaquest_data_providers = functools.partial(
    pong_data_providers, filename="./data/atari/Seaquest-100000-dqn-dec.h5"
)
qbert_data_providers = functools.partial(
    pong_data_providers, filename="./data/atari/Qbert-100000-dqn-dec.h5"
)
breakout_data_providers = functools.partial(
    pong_data_providers, filename="./data/atari/Breakout-100000-dqn-dec.h5"
)
beamrider_data_providers = functools.partial(
    pong_data_providers, filename="./data/atari/BeamRider-100000-dqn-dec.h5"
)
...
class Dataset(object):
    def __init__(self, args):
        crop_resize = args.crop_size if (args.crop_resize is None) else args.crop_resize
        ...
        if args.dataset == "mnist":
            ...
            providers = mnist_data_providers(
                args.batch_size, crop_size=crop_sizes, use_test_set=args.use_test_set
            )
            ...
            if args.raw_size and os.path.exists(presized_root):
                ...
                print(
                    "Pre-sized data not found (%s); using original data: %s"
                    % (presized_root, root)
                )
            if args.include_labels:
                labels = list(
                    set([int(l) for l in args.include_labels.split(",") if l])
                )
                labels.sort()
            if args.max_labels and args.include_labels:
                ...
                if len(labels) > max_labels:
                    print(
                        (
                            "Warning: both --max_labels and --include_labels "
        ...
        all_providers = [
            providers[k] if k in providers else None for k in ("train", "val", "test")
        ]
        self.ntrain, self.nval, self.ntest = [
            0 if p is None else len(p.data) for p in all_providers
        ]
